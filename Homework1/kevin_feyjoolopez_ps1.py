...
croppedImage1 = normalizedImage1[0:720, 0:int(652/2)]
croppedImage2 = normalizedImage2[:, int(652/2):]
newImage = np.concatenate((croppedImage1, croppedImage2), axis=1)

#e)	Using a for loop, create a new image such that every odd numbered row is the corresponding row from image1 and the every even row is the corresponding row from image2 
# (Hint: Remember that indices start at 0 and not 1 in Python). 
# Display this image in your report.
emptyImage = np.empty((720, 652))
for row in range(len(normalizedImage1)):
    for rowi in range(len(normalizedImage2)):
        if row % 2 == 1: #odd
            emptyImage[row] = normalizedImage1[row]

        else: #even
            emptyImage[row] = normalizedImage2[rowi]
            #something is happening with Normalizedimage2... comment this section out to let rest of code run
...
emptyImage2 = np.empty((720, 652))
emptyImage2[1::2] = oddMatrix
emptyImage2[::2] = evenMatrix

#g)	Convert the result from part f) to a grayscale image. Display the grayscale image with a title in your report.
plt.imshow(emptyImage2, cmap=plt.cm.gray)
